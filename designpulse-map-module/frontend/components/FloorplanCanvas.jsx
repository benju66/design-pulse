...
import MappedZone from '@/components/canvas/MappedZone';
...
const FloorplanCanvas = forwardRef(({
  ...
  // Use localized vector tree per constraints
      ...
      if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key) && selectedZoneIdsRef.current?.length > 0 && !isInputActive) {
        ...
        if (currentLayout && currentLayout.drawW && currentLayout.drawH) {
          ...
          activeIds.forEach(id => {
            ...
            if (zone && zone.polygon_coordinates) {
  ...
  const visibleZones = useMemo(() => {
    ...
    return zones.filter(zone => {
      if (!zone.polygon_coordinates || zone.polygon_coordinates.length === 0) return true;
      
      return zone.polygon_coordinates.some(pt => 
        pt.pctX >= minPctX && 
        pt.pctX <= maxPctX && 
        pt.pctY >= minPctY && 
        pt.pctY <= maxPctY
  ...
  const handleStageClick = (e) => {
    ...
    if (toolMode === 'stamp' && selectedZoneIds?.length === 1) {
      ...
      if (sourceZone && sourceZone.polygon_coordinates && sourceZone.polygon_coordinates.length > 0) {
        let sumX = 0, sumY = 0;
        sourceZone.polygon_coordinates.forEach(pt => { sumX += pt.pctX; sumY += pt.pctY; });
        const cx = sumX / sourceZone.polygon_coordinates.length;
        const cy = sumY / sourceZone.polygon_coordinates.length;
        const dx = pctX - cx;
        const dy = pctY - cy;
        
        const translatedPoints = sourceZone.polygon_coordinates.map(pt => ({
          pctX: pt.pctX + dx,
          pctY: pt.pctY + dy
        }));
  ...
  const handlePolygonClick = (e, zone) => {
  ...
  const handleFlip = (direction) => {
    ...
    if (!zone || zone.polygon_coordinates.length === 0) return;
  ...
  const handleRotatePolygon = (direction, overrideId = null) => {
    ...
    if (!zone || !zone.polygon_coordinates || zone.polygon_coordinates.length === 0) return;
  ...
  const handlePolygonDragEnd = (e, zone) => {
  ...
  const handleAnchorDragEnd = (e, zoneId, index) => {
    ...
    const newPoints = [...zone.polygon_coordinates];
    newPoints[index] = { pctX, pctY };
    onUpdateZonePolygon?.(zoneId, newPoints);
  };

  const handleAnchorClick = (e, zoneId, index) => {
    e.cancelBubble = true;
    if (toolMode !== 'delete_node') return;
    const zone = zones.find(z => z.id === zoneId);
    if (!zone || zone.polygon_coordinates.length <= 3) return;
  ...
  return (
    ...
    >
      ...
      {settings?.showHistoryHover && hoveredZone && pointerPos && !contextMenu && !['draw', 'add_node'].includes(toolMode) && (
        (() => {
          const z = zones.find(x => x.id === hoveredZone);
          return (
            <div
            ...
            >
              <div className="font-bold text-base mb-2 border-b border-slate-700/50 dark:border-black/10 pb-2 flex justify-between items-center">
                 <span>{z?.zoneId || z?.name || 'Unknown Zone'}</span>
                 {z?.type && (
              ...
              <div className="flex flex-col gap-2.5 max-h-[250px] overflow-y-auto pr-2" style={{ scrollbarWidth: 'thin' }}>
                 {z?.tooltipItems ? (
                    z.tooltipItems.map((item, idx) => (
                       <div key={idx} className={`flex items-center justify-between gap-4 text-xs opacity-100`}>
                         <div className="flex items-center gap-2 truncate">
                           {item.color && <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: item.color }} />}
                           <span className="truncate font-medium">{item.label}</span>
                         </div>
                         <span className={`text-[9px] uppercase tracking-widest font-bold shrink-0 text-${item.valueColor || 'slate-400'}`}>
                           {item.value}
                         </span>
                       </div>
                    ))
                 ) : (
                    <div className="text-xs italic opacity-50">No properties mapped.</div>
                 )}
              </div>
            </div>
          );
