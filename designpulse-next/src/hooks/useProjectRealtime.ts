  ...
  useEffect(() => {
    ...
    const debouncedInvalidate = () => {
      ...
      timeoutId = setTimeout(() => {
        ...
        queryClient.invalidateQueries({ queryKey: ['ve-scenarios', projectId], refetchType: 'active' });
      }, 300);
    };
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 've_scenarios', filter: `project_id=eq.${projectId}` },
        debouncedInvalidate
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 've_scenario_packages', filter: `project_id=eq.${projectId}` },
        debouncedInvalidate
      )
      .subscribe();
