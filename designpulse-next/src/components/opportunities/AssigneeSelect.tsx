...
export function AssigneeSelect({ value, onChange, onClose, members, autoFocus }: AssigneeSelectProps) {
  ...
  // Click outside listener — native containment check (SKILL.md Rule C16)
  // Replaces the fragile onBlur container check which prematurely closes WebKit dropdowns on click
  useEffect(() => {
    if (!isOpen) return;
