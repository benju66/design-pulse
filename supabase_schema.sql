...
CREATE TABLE IF NOT EXISTS project_sequences (
  ...
  ll_current_value integer DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS item_activity (
  ...
  lesson_id uuid REFERENCES project_lessons(id) ON DELETE CASCADE,
...
-- ── Phase 3: Budget Detail Panel — Scope & Assumptions RPC ──────────────────
-- Allows users with can_edit_records to update item_assumptions on project_estimates
-- without granting broad write access to all estimate columns (unit_cost, budget_amount, etc.).
-- SECURITY DEFINER bypasses the can_edit_project_settings RLS gate on project_estimates
-- after an explicit RBAC check for can_edit_records. (Audit Fix #1)
...
-- 2. Junction Table: Lessons <-> Opportunities
