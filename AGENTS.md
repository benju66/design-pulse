...
* **JSONB Type Casting & Safe Iteration in RPCs:** When iterating over JSONB objects in PL/pgSQL using `FOR k, v IN SELECT key, value FROM...`, strictly use `jsonb_each_text()` instead of `jsonb_each()` if you intend to compare the extracted values against standard SQL text/strings (e.g., `IF v = 'true' THEN`). Using `jsonb_each()` causes fatal `operator does not exist: text = jsonb` errors. **CRITICAL:** Always wrap the iteration in a type check (`IF jsonb_typeof(my_jsonb_col) = 'object' THEN`) or use a `CASE WHEN jsonb_typeof(col) = 'object'` fallback in `FROM` clauses. Passing a JSON scalar (`'null'::jsonb`), an array, or a SQL `NULL` directly into `jsonb_each_text()` will cause a fatal Postgres crash.
