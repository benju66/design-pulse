...
export default function MyDeskDashboard({ projectId, opportunities }: MyDeskDashboardProps) {
  ...
  const myEstimates = pendingEstimates.filter(opp => {
    if (!opp.estimator_assignee) return true; // Show unassigned to everyone for triage
    const assignees = (opp.estimator_assignee || '').split(',').map(s => s.trim());
    return assignees.includes(email) || (displayName && assignees.includes(displayName));
  });
    ...
    <div className="flex-1 flex flex-col p-4 md:p-6 bg-slate-50 dark:bg-slate-950 h-full overflow-hidden">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {myEstimates.length === 0 ? (
                  ...
                  <tr key={opp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    ...
                    <td className={`px-4 py-3 text-right font-semibold ${Number(opp.cost_impact) < 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => {
                            if (confirm('Return this item to the Design Team for review? This will unlock the variance and set it back to Draft.')) {
                              updateMutation.mutate({ 
                                id: opp.id, 
                                updates: {
                                  estimate_sync_status: 'Draft', 
      ...
      {reconcileOppId && (
        <ReconcileValueModal
          ...
          pendingVariance={Number(myEstimates.find(o => o.id === reconcileOppId)?.cost_impact) || 0}
        />
      )}
    </div>
  );
